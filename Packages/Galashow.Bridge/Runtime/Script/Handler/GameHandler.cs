...
public sealed class GameHandler : BaseMessageHandler
{
    ...
    public void SendLoadingProgress(float progress, string currentTask)
    {
        var data = new Notify.U2R.LoadingProgress(progress, currentTask);
        NTY("LoadingProgress", data);
    }
    ...
    public override void HandleRequest(Message message, Action<object> onSuccess, Action<string> onError)
    {
        ...
        {
            case "Initialize":
                if (!Util.TryTo<Request.R2U.Initialize>(message.data, out var req, out var parseErr))
                {
                ...
                bool replied = false;
                void Reply(Acknowledge.U2R.Initialize resp)
                {
                ...
                foreach (var p in _ports) p.Initialize(req, Reply, Fail);
                break;
            default:
